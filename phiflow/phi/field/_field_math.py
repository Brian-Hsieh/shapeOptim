...
def spatial_gradient(field: CenteredGrid,
    ...
    * `type=StaggeredGrid` computes the spatial_gradient at face centers of neighbouring cells
    ...
    if type == CenteredGrid:
        values = math.spatial_gradient(field.values, field.dx.vector.as_channel(name=stack_dim.name), difference=difference, padding=field.extrapolation, stack_dim=stack_dim)
